...
- sample_area: (id:int, number:int, parcel_id:int, lat:real, lng:real,
  altitude_m:int, r_m:int, note:string nullable)
...
- sample_grid: (id:int, description:string)
  - Denotes a set of sample areas. We use the name `grid` because the sample
    areas are typically laid out on a uniformly spaced rectangular grid
...
- sample_area_grid: (sample_area_id:int, sample_grid_id:int)
  - Maps sample areas to their grid. PK is (sample_area_id, sample_grid_id).
...
- survey: (id:int, harvest_plan_id:int nullable, sample_grid_id:int,
  description:string)
  - Represents a high-level survey operation, consisting of one or more samples.
  - It is always associated with a `sample_grid` of physical sample areas.
  - It may or may not be associated with a `harvest_plan`.
