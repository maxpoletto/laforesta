...
marked. Normally only high-forest parcels undergo marking; region-wide
damaged/sick interventions may include marks in parcels that are technically
coppice, while coppice-only plan items are handled separately.
...
A `Totali` footer row in the high-forest table sums `Volume previsto`, `Volume
martellato`, and `Volume effettivo` across the complete current search result,
not just the visible pagination page. Each value is aligned under its source
column; an empty search result shows zero for all three totals. The coppice
table has no totals row.
