...
For the **code** architecture behind these maps — the `map-common.js` Leaflet
shim, the `ParcelMap` abstraction, the opt-in `map-tools.js`, and the parcel
data / hover-tooltip / view-persistence mechanisms — see
[`ui-maps.md`](ui-maps.md). This section covers only the visual rules.

**Basemap selector.** A 64×64 thumbnail of the active basemap (OSM / Topo /
Satellite, default Satellite). Hover/tap expands to show all three; click
selects. Present on every map; the choice persists in the `mt=` URL param.

**Parcel style.** All maps share one look — a warm yellow border with low fill
opacity, so the imagery shows through. Don't inline a style block in a new map
...
- **`table.js`**: `TableWrapper` owns filtering, sorting, pagination, and
  ...
  `renderSummaryRow(row, filteredRows, columns)`. The summary is a
  column-aligned table footer built from the complete filtered row set, so it follows search and
...
.  base/templates/base/_shell_templates_it.htm. .   — shared (confirm, cascade-delete)
