...
- `branding_out_dir` — optional; when set, receives `logo.png`,
  ...
  directory. The Abies checkout expects these under `data/branding` before
  build/deploy. Satellite data is copied by the Makefile, not by
  `convert_laforesta.py`.
...
The default target writes Abies bootstrap data, branding inputs, and satellite
data:
...
By default this writes canonical CSVs to `../abies/data/canonical`, branding
assets to `../abies/data/branding`, and copies legacy satellite files from
`../bosco/data/satellite` to `../abies/data/satellite`. Override `DST_DIR`,
`BRANDING_DST_DIR`, `SATELLITE_SRC_DIR`, or `SATELLITE_DST_DIR` if needed.
...
| `../abies/data/satellite/*` | `../bosco/data/satellite/*` | copied by `make satellite`; consumed at runtime from Abies' `data/satellite` and staged to the VM separately from canonical CSVs |
