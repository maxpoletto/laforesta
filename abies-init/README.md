...
| `particelle.csv` | `particelle.csv` | select/reorder `Compresa,Comparto,Particella,Area (ha),Età media,Località,Altitudine min,Altitudine max,Esposizione,Pendenza %,Stazione,Soprassuolo,Piano del taglio,Intervallo,Matricine`; `Parametro` is renamed to `Intervallo`; `Intervallo`/`Matricine` are emitted only for coppice (`Comparto=F`) and blanked for high forest; drop CP and Governo |
