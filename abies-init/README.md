...
- `branding_out_dir` — optional; when set, receives `logo.png`,
  ...
  build/deploy. Satellite data is generated by Abies, not by this converter.
...
The default target writes Abies bootstrap data and branding inputs:

```sh
make
```

By default this writes canonical CSVs to `../abies/data/canonical` and
branding assets to `../abies/data/branding`. Override `DST_DIR` or
`BRANDING_DST_DIR` if needed.
...
| `particelle.csv` | `particelle.csv` | select/reorder `Compresa,Comparto,Particella,Area (ha),Età media,Località,Altitudine min,Altitudine max,Esposizione,Pendenza %,Stazione,Soprassuolo,Piano del taglio,Intervallo,Matricine`; `Area (ha)` is normalized to dot decimals; legacy `Parametro` or current `Intervallo` becomes canonical `Intervallo`; `Intervallo`/`Matricine` are emitted only for coppice (`Comparto=F`) and blanked for high forest; drop CP and Governo |
...
| `sampled-trees.csv` | `alberi-calcolati.csv` + `alberi-altezze.csv` | union of the two surveys with explicit `H_measured` provenance (see below) |
...
`Rilevamento,Compresa,Particella,Area saggio,Albero,Pollone,Matricina,D_cm,H_m,H_measured,L10_mm,Pressler,Genere,Fustaia`:

1. **`Campionamento PDG Sabatino`** (from `alberi-calcolati.csv`): `n`→`Albero`,
   `poll`→`Pollone`, `D(cm)`→`D_cm`, `h(m)`→`H_m`, `L10(mm)`→`L10_mm`,
   `Pressler` is set to `2`, `H_measured` to `false`, plus `Genere`,
   `Fustaia`.
...
2. **`Campionamento altezze Luca`** (from `alberi-altezze.csv`): `D(cm)`→`D_cm`,
   ...
   - `Pollone`, `Matricina`, `L10_mm` are left **blank** (strict core defaults
     blank Pollone/L10 → 0, blank Matricina → False), `Pressler` is set to
     `2`, and `H_measured` is set to `true`.
