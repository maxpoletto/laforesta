...
\begin{flushleft}
Corigliano-Rossano (CS), li 21.04.2026
\end{flushleft}
...
Da notare che la provvigione elencata nella sezione~\ref{sec:provvigione} esclude le piante sottomisura, al di sotto dei 17,5 cm di diametro (classe diametrica inferiore a 20 cm), coerentemente con i volumi usati per il calcolo della ripresa e degli interventi di taglio.
...
Inoltre, per fustaie fino a 60 anni di età, il prelievo non può superare una determinata percentuale dell'area basimetrica, $g$, secondo la seguente tabella:
...
Per ottemperare a questi duplici requisiti, si procede con una simulazione albero per albero basata sulle piante campionate. Per ogni particella~$p$ di area $a$~(ha):

\begin{enumerate}
\item Si calcola il volume totale $v$ (m$^3$) delle piante mature (vedere sezione~\ref{sec:note-volumi}) e l'area basimetrica totale $g$ (m$^2$) delle stesse.

\item In base alla classe economica di $p$ e alla provvigione $v/a$, dalle prime due tabelle sopra si ricava il prelievo massimo percentuale $pp_{\max}$, e si pone il prelievo massimo assoluto $V_{\max} = v \cdot pp_{\max}/100$.

\item In base all'età media di $p$, dalla terza tabella si ricava il limite percentuale di area basimetrica $gp_{\max}$, e si pone il limite assoluto $G_{\max} = g \cdot gp_{\max}/100$.

\item Le piante mature campionate si ordinano per classe diametrica crescente (dai 20~cm in su), in modo da simulare un taglio colturale dal basso volto a rafforzare le piante di maggiori dimensioni.

\item Si scorrono le piante nell'ordine stabilito, accumulando il volume e l'area basimetrica (scalati per $s$, come sopra nella sezione~\ref{sec:note-volumi}) di ciascun albero $b$. Per ogni albero $b$, se l'aggiunta di $b$ non farebbe superare né $V_{\max}$ né $G_{\max}$, lo si include nel prelievo; altrimenti la simulazione si arresta. Il volume accumulato al termine costituisce il prelievo massimo per la particella.
\end{enumerate}
...
\section{Stima della provvigione}
...
\noindent Corigliano-Rossano (CS), li 21.04.2026
