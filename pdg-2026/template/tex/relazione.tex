\documentclass[a4paper,11pt]{article}
\usepackage[utf8]{inputenc}
\usepackage[italian]{babel}
\usepackage{amsmath}
\usepackage{cancel}
\usepackage{url}
\usepackage{graphicx}
\usepackage{geometry}
\usepackage{hyperref}
\usepackage{float}
\usepackage{longtable}
\usepackage{verbatim}
\usepackage{parskip}
\geometry{margin=2cm}
\setlength{\LTleft}{1cm}

% Typography: serif body text
\usepackage{libertinus}
\usepackage[T1]{fontenc}
% Sans-serif section titles (Gill Sans-like)
\usepackage{gillius}
\usepackage{sectsty}
\allsectionsfont{\sffamily}
\usepackage[style=alphabetic,backend=biber]{biblatex}
\addbibresource{ref.bib}

\newif\ifincludesections \includesectionstrue
\newcommand{\maybeinput}[1]{\ifincludesections \input{#1} \fi}
\newcommand{\pubdate}{03.08.2026}
\begin{document}

\begin{titlepage}
\rmfamily
\centering
\vspace*{1.5cm}

{\LARGE SOCIETÀ AGRICOLA LA FORESTA}
\vspace{0.8cm}

{\large Comuni di Serra San Bruno -- Fabrizia -- Capistrano}
\vspace{1.2cm}

{\LARGE Piano di Gestione Forestale}
\vspace{0.7cm}

{\large
Regione Calabria Protocollo 488277 REGCAL del 06/11/2023\\
Regione Calabria Aoo REGCAL Prot.\ n.\ 464707 del 25/06/2025
}
\vspace{1.2cm}

{\large
DOCUMENTAZIONE INTEGRATIVA E RISPOSTE ALLE NOTE ED OSSERVAZIONI\\
DELL'ACCADEMIA ITALIANA DI SCIENZE FORESTALI
}
\vspace{1.8cm}

{\fontsize{36}{45}{\rm RELAZIONE}}
\vfill

\begin{flushleft}
Corigliano-Rossano (CS), li \pubdate
\end{flushleft}

\vspace{0.4cm}

\begin{flushright}
\begin{tabular}{c}
Roberto Sabatino\\
Dott.\ Agronomo e Dott.\ Forestale
\end{tabular}
\end{flushright}

\end{titlepage}

\section{Introduzione}

Il presente documento contiene la documentazione integrativa al Piano di Assestamento della Società Agricola La Foresta S.R.L., come richiesto dal Dipartimento Politiche della Montagna, Foreste, Forestazione e Difesa del Suolo della Regione Calabria con Protocollo 464707 del 25/06/2025.

La relazione si articola in 9 parti.
\begin{itemize}
\item La parte~\ref{sec:note-metodologia} chiarisce alcuni metodi usati per redigere il piano, in particolare per quanto riguarda il calcolo degli incrementi e l'elaborazione del programma di taglio.
\item La parte~\ref{sec:ipsometrie} risponde al punto 1 della richiesta della regione: illustra per ogni specie principale le relative curve ipsometriche con equazioni interpolanti e la distribuzione grafica in classi diametriche del numero di alberi per ettaro.
\item La parte~\ref{sec:provvigione} risponde al punto 2 della richiesta: presenta la provvigione dei soprassuoli, calcolata usando le tavole prescritte dalle linee guida.
\item La parte~\ref{sec:metodo} descrive il metodo di gestione forestale praticato dall'azienda, sia per le fustaie che per i cedui.
\item La parte~\ref{sec:ripresa} risponde al punto 3 della richiesta: quantifica la ripresa in relazione alla provvigione.
\item La parte~\ref{sec:piano} presenta un piano di tagli aggiornato, redatto tenendo conto degli incrementi secondo i criteri descritti nella parte~\ref{sec:note-metodologia} e il metodo di gestione descritto nella parte~\ref{sec:metodo}.
\item La parte~\ref{sec:ceduo} risponde al punto 4 della richiesta: espone il piano di regolarizzazione dei cedui di castagno.

\item La parte~\ref{sec:particelle}, infine, risponde ai punti 5--7 della richiesta. Per ogni particella, indica le necessità colturali dei soprassuoli, descrive gli interventi colturali da eseguire, e fornisce, per ogni specie e classe diametrica, il numero di alberi per ettaro, il volume e l'area basimetrica per ettaro, la distribuzione delle altezze, e gli incrementi percentuali e correnti. Espone inoltre la provvigione e la ripresa totali, suddivise per specie.
\end{itemize}

\section{Note metodologiche}
\label{sec:note-metodologia}

Questa sezione chiarisce alcuni aspetti dei rilevamenti e dei metodi di calcolo usati nella stesura della relazione.

\subsection{Campionamento}

Le 177 aree di saggio già descritte nella relazione iniziale, ciascuna della dimensione di $\frac{1}{8}$ ha, sono state soggette a cavallettamento totale, fornendo i diametri di circa 9,900 piante. Per un sottoinsieme casuale di 1,880 piante è stata rilevata anche l'altezza tramite ipsometro laser. Per un ulteriore sottoinsieme di circa 200 piante è stato misurato l'incremento periodico di raggio $L_{10}$, ovvero lo spessore dei 10 anelli legnosi periferici successivi.

\subsection{Calcolo delle curve ipsometriche}
\label{sec:note:ipsometrie}
Le curve ipsometriche sono state calcolate per regressione sulle altezze e i diametri delle 1880 piante misurate, per ogni combinazione di specie e compresa comprendente più di 10 alberi campione. Le equazioni interpolanti hanno forma logaritmica, $h = a \ln d + b$, dove $d$ è il diametro in cm e $h$ è l'altezza in metri.

\subsection{Calcolo dei volumi}
\label{sec:note-volumi}

I volumi presenti in ciascuna particella $p$ sono stimati secondo la metodologia alle pp.20--27 di Tabacchi~\cite{tabacchi:2011}. Nello specifico:

\begin{enumerate}
    \item Per ogni albero cavallettato ma non misurato in altezza, l'altezza è stimata usando le equazioni interpolanti di cui sopra.
    \item Per un qualsiasi albero di specie $f$, il volume del fusto e dei rami grossi viene calcolato usando un'equazione di previsione della forma $v = b_{f0} + b_{f1}d^2h + b_{f2}d$, dove i coefficienti $b_{fi}$ provengono dal Tabacchi. Il volume totale di tutti gli $n_f$ alberi della specie $f$ campionati nella particella $p$ è quindi dato da $v_f = b_{f0}n_f + b_{f1}\sum_{i=1}^{n_f} d_i^2h_i + b_{f2}\sum_{i=1}^{n_f} d_i$. Analogamente, il margine di errore $e_f$ della stima di questo volume complessivo è calcolato usando le matrici di varianza e i coefficienti del Tabacchi.
    \item Il volume totale degli alberi campionati è dato quindi da $v_c = \sum_{f} v_f$. Il margine di errore è calcolato prudenzialmente come $e_c = \sum_{f} e_f$: ciò presume che le piante delle diverse specie nella particella siano perfettamente correlate, nonostante le equazioni di previsione di Tabacchi siano state calibrate sulla base di campioni indipendenti.
    \item Si definisce quindi un fattore di scala $s = a / a_s$, dove~$a$ è la superficie di~$p$ e~$a_s$ è la superficie complessiva delle aree di saggio in~$p$.
    \item Il volume dell'intera particella viene calcolato scalando il volume stimato nelle aree di saggio, $v = s\cdot v_c$, e il margine di errore, $e = s\cdot e_c$. Pure questo margine di errore è prudenziale, nel senso che presume che tutte le superfici di una particella siano correlate tra di loro. I limiti degli intervalli fiduciari al 95\% elencati nella sezione~\ref{sec:provvigione} corrispondono a $v \pm e$.
\end{enumerate}

La provvigione  nella sezione~\ref{sec:provvigione} esclude le piante
sottomisura, al di sotto dei 17,5 cm di diametro (classe diametrica inferiore a
20 cm), coerentemente con i volumi usati per il calcolo della ripresa e degli
interventi di taglio.

\subsection{Calcolo della ripresa}
\label{sec:note-ripresa}

Secondo le prescrizioni di massima della Regione Calabria~\cite{calabria:2011}, artt. 48 e 49, il prelievo durante i tagli colturali nelle fustaie è limitato in funzione della massa disponibile e dell'area basimetrica di ciascuna particella.

In base alla classe economica del comparto, si stabilisce una provvigione minima. Per la Società Agricola La Foresta, le classi economiche sono le seguenti:

\begin{center}
    \small
    \begin{tabular}{llr}
    \hline
    Classe           &                                          & Provvigione \\
    economica        & Caratteristiche                          & minima (m³/ha)\\
    \hline
    A                & Fustaia di abete                         & 350 \\
    B                & Fustaia mista di conifere e latifoglie   & 350 \\
    C                & Fustaia di pino                          & 250 \\
    D                & Fustaia di pino nero e latifoglie        & 250 \\
    E                & Fustaia di faggio e conifere             & 350 \\
    \hline
    \end{tabular}
\end{center}

Il prelievo massimo volumetrico è quindi dato dalla seguente tabella:

\begin{center}
    \small
    \begin{tabular}{lr}
    \hline
    Eccedenza su provvigione minima         & Prelievo massimo \\
    \hline
    $\ge 80\%$                              & 25\% \\
    $\ge 60\%, < 80\%$                      & 20\% \\
    $\ge 40\%, < 60\%$                      & 15\% \\
    $\ge 20\%, < 40\%$                      & 10\% \\
    $< 20\%$                                &  0\% \\
    \hline
    \end{tabular}
\end{center}

Nelle fustaie fino a 60 anni di età, dove si opera con tagli intercalari e
sfollamenti, il prelievo è limitato dall'area basimetrica $g$, secondo la
seguente tabella:

\begin{center}
    \small
    \begin{tabular}{lr}
    \hline
    Età media                   & Limite area basimetrica \\
    \hline
    $\ge 30$ anni, $< 60$ anni  & 20\% \\
    $<30$ anni                  & 15\% \\
    \hline
    \end{tabular}
\end{center}

Le prescrizioni regionali non chiariscono esplicitamente se i due limiti debbano
intendersi cumulativi o alternativi. La differenza è rilevante solo nelle
fustaie giovani: in questi soprassuoli la provvigione è spesso inferiore al
minimo di classe, e una lettura cumulativa precluderebbe ogni intervento, mentre
sfollamenti e diradamenti sono necessari per indirizzare la selezione verso i
soggetti meglio conformati (sezione~\ref{sec:metodo}). Si è pertanto adottata la
lettura alternativa, applicando in questi casi il solo limite di area
basimetrica, che resta osservato integralmente.

Nel presente piano ricadono in questa condizione le particelle Fabrizia 11c e
Capistrano 12b, 13, 15, 16b e 17b, le cui provvigioni sono riportate nella
sezione~\ref{sec:provvigione}.

Per ottemperare ai requisiti appena illustrati, si procede con una simulazione
albero per albero basata sulle piante campionate. Per ogni particella~$p$ di
area $a$~(ha):

\begin{enumerate}
\item Si calcola il volume totale $v$ (m$^3$) delle piante mature (vedere
sezione~\ref{sec:note-volumi}) e l'area basimetrica totale $g$ (m$^2$) delle
stesse.

\item In base alla classe economica di $p$ e alla provvigione $v/a$, dalle prime
due tabelle sopra si ricava il prelievo massimo percentuale $pp_{\max}$, e si
pone il prelievo massimo assoluto $V_{\max} = v \cdot pp_{\max}/100$. Per le
fustaie di età media inferiore a 60 anni, il limite volumetrico non si applica:
si pone quindi $V_{\max} = \infty$.

\item In base all'età media di $p$, dalla terza tabella si ricava il limite
percentuale di area basimetrica $gp_{\max}$, e si pone il limite assoluto
$G_{\max} = g \cdot gp_{\max}/100$. Per le particelle di età media pari o
superiore a 60 anni il limite di area basimetrica non si applica: si pone quindi
$G_{\max} = \infty$.

\item Le piante mature campionate si ordinano per classe diametrica crescente
(dai 20~cm in su), in modo da simulare un taglio colturale dal basso volto a
rafforzare le piante di maggiori dimensioni.

\item Si scorrono le piante nell'ordine stabilito, accumulando il volume e
l'area basimetrica (scalati per $s$, come sopra nella
sezione~\ref{sec:note-volumi}) di ciascun albero $b$. Per ogni albero $b$, se
l'aggiunta di $b$ non farebbe superare né $V_{\max}$ né $G_{\max}$, lo si
include nel prelievo; altrimenti la simulazione si arresta. Il volume accumulato
al termine costituisce il prelievo massimo per la particella.
\end{enumerate}

\subsection{Calcolo degli incrementi}

Per un sottoinsieme di alberi campione l'incremento periodico di raggio $L_{10}$ (spessore degli ultimi 10 anelli legnosi, in mm) viene misurato tramite carotaggio o abbattimento. La formula di Pressler fornisce l'incremento percentuale di volume dell'$i$-esimo albero, $pv_i$. Definendo l'incremento corrente di diametro in cm anno$^{-1}$

\[ \Delta d_i = \frac{2L_{10}}{100} \]

si ha

\[ pv_i = c \cdot \frac{100\Delta d_i}{d_i} = c \cdot \frac{2L_{10}}{d_i} \]

dove $d_i$ è il diametro (cm) e $c$ è il coefficiente di Pressler. Prudenzialmente si pone $c = 2$ per tutta la superficie boscata, nonostante studi svolti sull'Appennino Calabrese suggeriscano valori più alti (\cite{marziliano:2011} riscontra coefficienti di Schneider $K$ nell'intervallo $415$--$715$; analiticamente $c = K / 200$).

Per ogni area assestamentale (particella o compresa) e specie, l'incremento percentuale medio $pv_j$ della classe diametrica $j$ è determinato come media ponderata sui volumi degli alberi carotati in quell'area e appartenenti a quella classe, come in~\cite{corona:2007}:

\[ pv_j = \frac{\sum_{i=1}^{m_j} pv_{ij}v_{ij}}{\sum_{i=1}^{m_j} v_{ij}} \]

dove $pv_{ij}$ è l'incremento percentuale di volume dell'$i$-esimo albero carotato della classe diametrica $j$, $v_{ij}$ è il suo volume calcolato come sopra alla sezione~\ref{sec:note-volumi}, e $m_j$ è il numero di alberi carotati nella classe $j$.

Il volume complessivo degli alberi nella classe $j$ è dato da

\[ V_j = s\cdot\sum_{i=1}^{n_j} v_{ij} \]

dove $n_j$ è il numero totale di alberi campionati nella classe $j$, e $s$ è un fattore di scala come sopra alla sezione~\ref{sec:note-volumi}. (Si noti che $n_j \gg m_j$.)

Quindi l'incremento percentuale $pV$ di tutti gli alberi nell'area assestamentale è la media ponderata sui volumi delle classi diametriche:

\begin{equation}
    \label{eq:pV}
    pV = \frac{\sum_{j=1}^{M} pv_jV_j}{\sum_{j=1}^{M} V_j}
\end{equation}
dove $M$ è il numero di classi diametriche e $V_j$ è il volume totale della classe $j$.

La ponderazione per volume fa sì che gli alberi di maggiori dimensioni, che contribuiscono di più alla massa legnosa, pesino di più nella stima dell'incremento di gruppo.

Equivalentemente, a ogni albero campione $i$ non carotato si può attribuire l'incremento percentuale della sua classe diametrica $j$, $pv_i = pv_j$, sostituendo l'equazione~\ref{eq:pV} con un calcolo ponderato sui volumi dei singoli alberi:
\begin{equation}
    \label{eq:pV2}
    pV = \frac{\sum_{i=1}^{N}pv_iv_i}{\sum_{i=1}^{N}v_i}
\end{equation}

dove $N$ è il numero di tutti gli alberi campionati nell'area assestamentale.

L'incremento corrente $\Delta V$ (m$^3$) di un gruppo di piante è dato quindi dal volume complessivo del gruppo moltiplicato per l'incremento percentuale medio:

\[ \Delta V = V \cdot \frac{pV}{100} \]

Si noti che per ottenere i volumi totali dell'area assestamentale i volumi effettivamente campionati vengono scalati in proporzione alla superficie campionata, come sopra nella sezione~\ref{sec:note-volumi}.

% Group the sum in eq:pV2 by diameter class $j$. The numerator becomes (ignoring $s$, which cancels):
%
% $$
% \sum_j \left[\underbrace{\sum_{i \in \text{cored}} pv_iv_i}_{\text{actual } pv_i} + \underbrace{pv_j \sum_{i \in \text{non-cored}} v_i}_{\text{assigned } pv_j}\right]$$
%
% For equivalence with eq:pV we need this to equal $\sum_j pv_j \sum_{i=1}^{n_j} v_i$, i.e.:
%
%  $$pv_j \sum_{i \in \text{cored}} v_i = \sum_{i \in \text{cored}} pv_iv_i$$
%
% But this is the definition of $pv_j$ at line 164, rearranged:
%
%  $$pv_j = \frac{\sum_{i=1}^{m_j} pv_iv_i}{\sum_{i=1}^{m_j} v_i} \quad \Longrightarrow \quad pv_j \sum_{i \in \text{cored}} v_i = \sum_{i \in \text{cored}} pv_iv_i \quad$$


\subsection{Aggiornamento del programma dei tagli}
\label{sec:note:piano}

Il programma di tagli proposto nella relazione originale ha due difetti: per diverse particelle propone interventi colturali (seppure al di sotto dei limiti stabiliti) a intervalli minori di 10 anni, e non tiene conto degli incrementi nel valutare l'entità di interventi futuri.

Entrambe le problematiche sono state affrontate tramite una simulazione a tempo discreto ispirata al metodo matriciale di Usher~\cite{usher:1966}, comunemente usato nella modellizzazione delle dinamiche forestali. La simulazione opera su un orizzonte temporale definito (ad es.\ 2026--2040) con passo annuale, e ad ogni anno seleziona le particelle da sottoporre a taglio tenendo conto sia della crescita avvenuta sia dei vincoli normativi.

\subsubsection{Parametri della simulazione}
\label{sec:note-parametri}

La simulazione richiede i seguenti parametri:
\begin{itemize}
\item un \emph{volume obiettivo} annuo $V_o$ (m$^3$), che rappresenta il prelievo complessivo desiderato dall'azienda in un dato anno, sempre che le condizioni di produttività del bosco lo permettano;
\item un \emph{intervallo minimo} $t$ (anni) tra due interventi successivi sulla stessa particella;
\item un tasso di \emph{mortalità} annuo $m$ (\%), applicato uniformemente a tutte le piante. In assenza di precisi dati stazionali, si adotta un tasso del 1\%~\cite{lacasia:2025}.
\item un \emph{coefficiente di prudenza} $c$ (\%), che riduce cautelativamente i limiti di prelievo ricavati secondo la sezione~\ref{sec:note-ripresa}: i limiti effettivamente applicati nella simulazione sono $V_{\max}\cdot c/100$ e $G_{\max}\cdot c/100$. Nel presente piano si adotta $c = 80\%$.
\end{itemize}

\subsubsection{Tabella di crescita}
Prima della simulazione, si costruisce una tabella di crescita a partire dagli incrementi osservati (sezione precedente). Per ogni combinazione di compresa, specie e classe diametrica, la tabella riporta l'incremento percentuale medio $pv$ e l'incremento corrente di diametro $\Delta d$. Quando per una data combinazione non sono disponibili dati diretti, si utilizza il valore della classe diametrica più vicina della stessa specie e compresa.

\subsubsection{Ciclo annuale}
\label{sec:note-ciclo}

Per ogni anno dell'orizzonte di simulazione:

\begin{enumerate}
    \item \textbf{Ordinamento.} Le particelle vengono ordinate per provvigione matura decrescente (m$^3$/ha), in modo da intervenire prioritariamente sui soprassuoli più densi, riducendone la competizione e permettendo ulteriore crescita in quelli meno densi.
    \item \textbf{Selezione e taglio.} Per ciascuna particella, nell'ordine stabilito:
    \begin{itemize}
        \item Se l'ultimo intervento è avvenuto meno di $t$ anni prima, la particella viene esclusa.
        \item Altrimenti, si simula un taglio colturale dal basso secondo le regole descritte nella sezione~\ref{sec:note-ripresa}. Gli alberi prelevati vengono rimossi dalla simulazione.
        \item Il volume prelevato si accumula nel totale annuo; quando questo raggiunge $V_o$, non si interviene in ulteriori particelle nell'anno corrente.
    \end{itemize}

    \item \textbf{Crescita.} Al termine dei tagli, tutte le piante rimanenti crescono di un anno:
    \begin{itemize}
        \item Il volume di ciascun albero viene aggiornato per crescita e mortalità:
        \[v \leftarrow v\cdot(1 + pv/100)\cdot(1 - m/100) \]
        Il tasso di crescita $pv$ è specifico di ciascun albero; il tasso di mortalità $m$, proprio della popolazione, viene modellato quale riduzione uniforme del volume di ogni individuo.
        \item Il diametro viene incrementato di $\Delta d$, con conseguente eventuale passaggio alla classe diametrica superiore.
        \item L'età media di ogni particella viene incrementata di un anno. (Nella realtà probabilmente l'aumento è superiore perché i tagli privilegiano le piante più piccole.)
    \end{itemize}
\end{enumerate}

Il risultato della simulazione è un programma dei tagli che indica, per ogni anno e particella, il volume prima dell'intervento, il prelievo e il volume residuo.

\subsubsection{Prudenzialità della simulazione}

L'ipotesi di taglio colturale dal basso menzionata nella
sezione~\ref{sec:note-ripresa} e~\ref{sec:note-ciclo} è uguale per tutte le
particelle, ma nella realtà, come dettagliato nella
sezione~\ref{sec:particelle}, le modalità di intervento variano in funzione
delle condizioni del soprassuolo.

Il taglio dal basso, tuttavia, è un criterio prudenziale per il calcolo della
ripresa: asportando le piante di minori dimensioni, che presentano il maggiore
accrescimento percentuale, riduce la capacità di accrescimento del soprassuolo
residuo. Nelle nostre simulazioni, una strategia diametricamente opposta, in cui
si prelevano per prime le piante di maggiori dimensioni, produce nell'arco del
piano una ripresa superiore di circa il 4\% e lascia al termine una provvigione
in piedi maggiore di circa il 2\%.

Coerentemente con il nostro approccio prudenziale, esemplificato anche dal
coefficiente di prudenza $c$ alla sezione~\ref{sec:note-parametri}, abbiamo
scelto di adottare il taglio dal basso come criterio di simulazione comune per
tutte le particelle.

\section{Curve ipsometriche e classi diametriche}
\label{sec:ipsometrie}
\input{sec-ipsometrie}

\setlength{\LTleft}{0.1in}

\section{Provvigione}
\label{sec:provvigione}
\input{sec-provvigione}

\section{Criteri di intervento selvicolturale}
\label{sec:metodo}
\input{sec-metodo}

\section{Ripresa attuale}
\label{sec:ripresa}
\input{sec-ripresa}

\clearpage
\section{Piano dei tagli}
\label{sec:piano}
\input{sec-piano}

\clearpage
\section{Piano dei cedui}
\label{sec:ceduo}
\input{sec-ceduo}

\maybeinput{sec-particelle}

\clearpage
\thispagestyle{empty}
\vspace*{\fill}

\noindent Corigliano-Rossano (CS), li \pubdate

\vspace{2cm}

\noindent Firma e timbro

\vspace{2.5cm}

\noindent\rule{7cm}{0.4pt}\\
Roberto Sabatino\\
Dott.\ Agronomo e Dott.\ Forestale

\vspace{3cm}

\clearpage
\printbibliography

\end{document}
