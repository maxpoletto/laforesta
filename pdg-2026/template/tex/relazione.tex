...
\begin{enumerate}
    ...
    \item Per tutti gli alberi di una singola specie $f$ campionati in $p$, il volume del fusto e dei rami grossi viene calcolato usando un'equazione di previsione della forma $v_f = b_1 + b_2d^2 + b_3d$, dove i coefficienti $b_i$ provengono dal Tabacchi. Analogamente, il margine di errore $e_f$ della stima di questo volume è calcolato usando le matrici di varianza e i coefficienti del Tabacchi.
    \item Il volume totale degli alberi campionati è dato quindi da $v_c = \sum_{f} v_f$. Il margine di errore corrispondente è calcolato come $e_c = \sqrt{\sum_{f} e_f^2}$, dove $e_f$ è il margine di errore di ciascuna specie $f$, perché le equazioni di previsione sono state calibrate sulla base di campioni indipendenti.
    ...
    \item Il volume dell'intera particella viene calcolato scalando il volume stimato nelle aree di saggio, $v = s\cdot v_c$, e il margine di errore $e = \sqrt{s}\cdot e_c$. Questo margine di errore è prudenziale, nel senso che presume che tutte le superfici di una particella siano correlate tra di loro.
\end{enumerate}
...
Per ogni area assestamentale (particella o compresa), procediamo alla determinazione dell'incremento percentuale medio $pv_j$ della classe diametrica $j$ quale media ponderata sui volumi dei singoli alberi campione, come in~\cite{corona:2007}: 
...
dove $M$ è il numero di classi diametriche e $V_j$ è il volume totale della $j$-esima classe diametrica,

\[ V_j = \sum_{i=1}^{n_j} s\cdot v_{ij} \] 

e $s$ è un fattore di scala come sopra alla sezione~\ref{sec:note-volumi}.

Algebraicamente ciò si riduce a

\[
    pV = \frac{\sum_{j=1}^{M}\left[\frac{\sum_{i=1}^{n_j} pv_{ij}v_{ij}}{\sum_{i=1}^{n_j}v_{ij}}\sum_{i=1}^{n_j} s\cdot v_{ij}\right]}{\sum_{j=1}^{M}\sum_{i=1}^{n_j} s\cdot v_{ij}}
       = \frac{\sum_{j=1}^{M}\left[s\cdot\frac{\sum_{i=1}^{n_j} pv_{ij}v_{ij}}{\sum_{i=1}^{n_j}v_{ij}}\sum_{i=1}^{n_j} v_{ij}\right]}{\sum_{j=1}^{M}s\cdot\sum_{i=1}^{n_j} v_{ij}}
       = \frac{\sum_{i=1}^{N}pv_iv_i}{\sum_{i=1}^{N}v_i}
\]

dove $N = \sum_{j}{n_j}$ è il numero di tutti gli alberi $i$ campionati nell'area assestamentale.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Il volume prelevato si accumula nel totale annuo; quando questo raggiunge $V_o$, non si effettuano ulteriori tagli nell'anno corrente.
    \end{itemize}

    \item \textbf{Crescita.} Al termine dei tagli, tutte le piante rimanenti crescono di un anno:
    \begin{itemize}
        \item il volume di ciascun albero viene aggiornato per crescita e mortalità: $v \leftarrow v\cdot(1 + pv/100)\cdot(1 - m/100)$. Il tasso di crescita $pv$ è specifico di ciascun albero; il tasso di mortalità $m$, proprio della popolazione, viene modellato quale riduzione uniforme del volume di ogni individuo;
